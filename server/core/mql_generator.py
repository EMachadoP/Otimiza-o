...
'''
    ...
    def _generate_mql(self, strategy: Dict, version: str) -> str:
        ...
        if strategy_type == 'trend':
            ...
            parameters.setdefault("BbPeriod", parameters.get("period", 20))
            parameters.setdefault("BbStd", parameters.get("std", 2.0))
        elif strategy_type == 'mean_reversion':
            parameters.setdefault("Period", parameters.get("period", 20))
            parameters.setdefault("StdDev", parameters.get("std", 2.0))
        elif strategy_type == 'scalping':
            parameters.setdefault("FastEMA", 5)
            parameters.setdefault("SlowEMA", 13)
            parameters.setdefault("RsiPeriod", 7)
        ...
        metadata_str = self._serialize_metadata(strategy, strategy_type, indicators)
        ...
        output = output.replace('{metadata}', metadata_str)
        ...
        return output
    def _normalize_strategy_type(self, strategy_type: Any) -> str:
        """Normaliza aliases de estratégia para os tipos suportados pelo exportador."""
        normalized = str(strategy_type or 'trend').strip().lower()
        aliases = {
            'ema_cross': 'trend',
            'macd': 'trend',
            'rsi': 'reversal',
    ...
    def _serialize_metadata(self, strategy: Dict, strategy_type: str, indicators: List[str]) -> str:
        """Embute metadados para permitir round-trip lossless no import/export."""
        payload = {
            'id': strategy.get('id'),
   ...
   return 0;'''
            ...
            return '''   double upper = iBands(Symbol(), PERIOD_CURRENT, InpBbPeriod, InpBbStd, 0, PRICE_CLOSE, MODE_UPPER, 0);
   ...
   if(close0 > upper) return 1;
   if(close0 < lower) return -1;
   return 0;'''

        elif strategy_type == 'mean_reversion':
            return '''   double upper = iBands(Symbol(), PERIOD_CURRENT, InpPeriod, InpStdDev, 0, PRICE_CLOSE, MODE_UPPER, 0);
   double lower = iBands(Symbol(), PERIOD_CURRENT, InpPeriod, InpStdDev, 0, PRICE_CLOSE, MODE_LOWER, 0);
   double close0 = iClose(Symbol(), PERIOD_CURRENT, 0);
   ...
   if(fastMA > slowMA && rsi > 55) return 1;
   if(fastMA < slowMA && rsi < 45) return -1;
   return 0;'''

        elif strategy_type == 'donchian':
            return '''   double highestHigh = iHigh(Symbol(), PERIOD_CURRENT, iHighest(Symbol(), PERIOD_CURRENT, MODE_HIGH, InpPeriod, 1));
   double lowestLow = iLow(Symbol(), PERIOD_CURRENT, iLowest(Symbol(), PERIOD_CURRENT, MODE_LOW, InpPeriod, 1));
   double close0 = iClose(Symbol(), PERIOD_CURRENT, 0);

   if(close0 > highestHigh) return 1;
   if(close0 < lowestLow) return -1;
   return 0;'''
